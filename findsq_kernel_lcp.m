function [sopt,qopt,costopt] = findsq_kernel_lcp(GammaK,p,normV,tol,d,shat)
%
% function [sopt,qopt,costopt] = findsq_kernel_lcp(GammaK,p,normV,tol,d,shat)
%
% Detect optimal scaling and number of quadrature points for linear
% combinations of phi-functions.
% detect max scaling
rho = max(abs(GammaK));
...
while ((s <= smax) && (q > qmin))
  ...
  if (any(isinf(fellZA(:))))
    ...
    Rqtemp = esterr_kernel_lcp(qtemp,p,GammaK,s,normV,tol,zbar,qbar,fellZA,z,dz);
  end
