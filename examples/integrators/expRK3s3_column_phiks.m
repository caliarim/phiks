...
% Method: exponential Runge Kutta of order 3
  ...
  for j = 1:ts
    ...
    [aux1,aux1half,aux1quarter,s1,q1] = phiks(tau,A,FtU,1,tol*normU,3);
    c1 = q1 + s1*1 + 3;
    U2 = U+c(2)*tau*aux1quarter{2};
    D2 = g(t+c(2)*tau,U2)-gtU;
    [aux2,aux2half,aux2quarter,s2,q2] = phiks(tau,A,D2,2,tol*normU,3);
    c2 = q2 + s2*2 + 3;
    U3 = U+tau*(c(3)*aux1half{2}+c(2)*gamma*aux2quarter{3}+...
                     c(3)^2/c(2)*aux2half{3});
    D3 = g(t+c(3)*tau,U3)-gtU;
    [aux3,s3,q3] = phiks(tau,A,D3,2,tol*normU,1);
    c3 = q3 + s3*2 + 1;
